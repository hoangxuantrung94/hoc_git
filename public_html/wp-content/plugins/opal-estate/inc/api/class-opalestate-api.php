...
class Opalestate_API {
	...
	private function missing_auth() {
		$error          = array();
		$error['error'] = esc_html__( 'You must specify both a token and API key!', 'opalestate' );
	...
	private function invalid_auth() {
		$error          = array();
		$error['error'] = esc_html__( 'Your request could not be authenticated!', 'opalestate' );
	...
	private function invalid_key() {
		$error          = array();
		$error['error'] = esc_html__( 'Invalid API key!', 'opalestate' );
	...
	private function invalid_version() {
		$error          = array();
		$error['error'] = esc_html__( 'Invalid API version!', 'opalestate' );
	...
	public function set_query_mode() {
		...
		if ( ! in_array( $query, $accepted ) ) {
			$error['error'] = esc_html__( 'Invalid query!', 'opalestate' );
		...
		if ( $agent == null ) {
			...
			if ( get_post_type( $agent ) == 'opalestate_property' ) {
				...
				$error['error'] = sprintf(
					...
					esc_html__( 'Form %s not found!', 'opalestate' ),
					$agent
				);
	...
	public function output( $status_code = 200 ) {
		...
		switch ( $propertyat ) :
			...
			case 'xml' :
				...
				$xml = Array2XML::createXML( 'opalestate', $this->data );
				echo $xml->saveXML();
	...
	function user_key_field( $user ) {
		...
		if ( ( opalestate_get_option( 'api_allow_user_keys', false ) || current_user_can( 'manage_opalestate_settings' ) ) && current_user_can( 'edit_user', $user->ID ) ) {
			...
			<table class="property-table">
				...
				<tr>
					<th>
						<?php esc_html_e( 'Opalestate API Keys', 'opalestate' ); ?>
					</th>
					<td>
						<?php
						...
						<?php if ( empty( $user->opalestate_user_public_key ) ) { ?>
							...
							<span class="description"><?php esc_html_e( 'Generate API Key', 'opalestate' ); ?></span>
						<?php } else { ?>
							<strong style="display:inline-block; width: 125px;"><?php esc_html_e( 'Public key:', 'opalestate' ); ?>&nbsp;</strong>
							<input type="text" disabled="disabled" class="regular-text" id="publickey" value="<?php echo esc_attr( $public_key ); ?>"/>
							<br/>
							<strong style="display:inline-block; width: 125px;"><?php esc_html_e( 'Secret key:', 'opalestate' ); ?>&nbsp;</strong>
							<input type="text" disabled="disabled" class="regular-text" id="privatekey" value="<?php echo esc_attr( $secret_key ); ?>"/>
							<br/>
							<strong style="display:inline-block; width: 125px;"><?php esc_html_e( 'Token:', 'opalestate' ); ?>&nbsp;</strong>
							<input type="text" disabled="disabled" class="regular-text" id="token" value="<?php echo esc_attr( $this->get_token( $user->ID ) ); ?>"/>
							<br/>
							<input name="opalestate_set_api_key" type="checkbox" id="opalestate_set_api_key" value="0"/>
							<span class="description"><label for="opalestate_set_api_key"><?php esc_html_e( 'Revoke API Keys', 'opalestate' ); ?></label></span>
						<?php } ?>
					</td>
				</tr>
				</tbody>
	...
	public function process_api_key( $args ) {
		...
		if ( ! wp_verify_nonce( $_REQUEST['_wpnonce'], 'opalestate-api-nonce' ) ) {
			...
			wp_die( esc_html__( 'Nonce verification failed.', 'opalestate' ), esc_html__( 'Error', 'opalestate' ), array( 'response' => 403 ) );
		...
		if ( empty( $args['user_id'] ) ) {
			wp_die( esc_html__( 'User ID Required.', 'opalestate' ), esc_html__( 'Error', 'opalestate' ), array( 'response' => 401 ) );
		}
		...
		if ( $user_id == get_current_user_id() && ! opalestate_get_option( 'allow_user_api_keys' ) && ! current_user_can( 'manage_opalestate_settings' ) ) {
			wp_die(
				sprintf(
					...
					esc_html__( 'You do not have permission to %s API keys for this user.', 'opalestate' ),
					$process
				),
				esc_html__( 'Error', 'opalestate' ),
				array( 'response' => 403 )
			);
