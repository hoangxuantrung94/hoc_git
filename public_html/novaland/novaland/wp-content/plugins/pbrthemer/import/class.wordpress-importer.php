...
define( 'PBR_THEMER_SAMPLE_CACHE_DIR', get_template_directory().'/inc/import/' );
/**
...
class WP_Import extends WP_Importer {
	...
	public function saveCaches(){
		foreach( $this->caches as $cache ){
			if( $this->$cache ){  
				file_put_contents( PBR_THEMER_SAMPLE_CACHE_DIR.$cache.'.txt', json_encode( $this->$cache ) );
			}
		}
	}
	...
	public function deleteCaches(){
		foreach( $this->caches as $cache ){
			if(  is_file( PBR_THEMER_SAMPLE_CACHE_DIR.$cache.'.txt' ) ){  
				@unlink(  PBR_THEMER_SAMPLE_CACHE_DIR.$cache.'.txt' );
			}
		}
	}
	public function getCaches(){
		foreach( $this->caches as $cache ){
			$this->$cache = json_decode( file_get_contents( PBR_THEMER_SAMPLE_CACHE_DIR.$cache.'.txt' ), true );
		}
	}
...
<?php
	...
	function process_posts() {
		...
		$posts = array_chunk( $this->posts, 40 );
		

		$_SESSION['importpostcount'] = count( $posts )+1; 

		if( !isset($posts[$_SESSION['importpoststart']]) ){
			$_SESSION['importpostcount']=$_SESSION['importpoststart'];
			return true; 
		}
		$this->posts = $posts[$_SESSION['importpoststart']]; 

		echo '----'.count($posts).'---'.$_SESSION['importpoststart'];
