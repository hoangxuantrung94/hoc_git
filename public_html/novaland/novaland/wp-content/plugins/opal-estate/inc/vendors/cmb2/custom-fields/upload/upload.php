...
class Opalestate_Field_Opal_Upload {
	...
	public static function init() { 
		...
		add_action( 'wp_ajax_opalestate_upload_images',  array(__CLASS__,'upload_image') );
		add_action( 'wp_ajax_nopriv_opalestate_upload_images',  array(__CLASS__,'upload_image') );

		add_action( 'wp_ajax_opalestate_upload_user_avatar',  array(__CLASS__,'upload_user_avatar') );
		add_action( 'wp_ajax_nopriv_opalestate_upload_user_avatar',  array(__CLASS__,'upload_user_avatar') );

		// add_action('wp_enqueue_scripts',  array("Opalestate_Field_Opal_Upload",'setup_admin_scripts'), 999 );
		add_action( 'wp_ajax_opalestate_delete_property_image' , array( __CLASS__, 'delete_property_image' ) );
		add_action( 'wp_ajax_nopriv_opalestate_delete_property_image' , array( __CLASS__, 'delete_property_image' )  );
	}
	...
	public static function upload_user_avatar(){
        ...
        if ( ! wp_verify_nonce( $nonce, 'allow_uploaded' ) || !$user_id ) {
            $ajax_response = array( 'success' => false , 'reason' => __('Security check failed!','opalestate') );
        ...
        do_action( 'opalestate_before_process_ajax_upload_user_avatar' );
	...
	public static function setup_admin_scripts() {   
		...
		$prop_data = array(
            ...
            'fileTypeTitle' => esc_html__( 'Valid file formats', 'opalestate' ),
            'message' => esc_html__( 'Please enter only digits', 'opalestate' )
        );
        wp_localize_script( 'opal-upload', 'opalestate', $prop_data );
	}
	...
	public static function delete_property_image(){
		...
		if( !isset($_POST['property_id'] , $_POST['attachment_id']) ){
			$ouput->message = __('Could not delete this?','opalestate');
		...
		if( !isset($property_images[$attchment_id]) ){
			$ouput->message = __('Could not found this in the collection?','opalestate');
			echo json_encode($ouput);exit;
		}
		if( !opalestate_is_own_property($post_id, $user_id) ){
			$ouput->message = __('The post is not owned by this user?','opalestate');
			echo json_encode($ouput);exit;
		}
		...
		$ouput->message = __( 'Deleted this done', 'opalestate' );
		$ouput->status  = true; 
